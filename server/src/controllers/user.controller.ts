...
export const signup = async (
  ...
  try {
    ...
    console.log("Unidentified error in signup:", error);
    return response.status(500).json({ msg: "Unidentified error occurred" });
  }
};
...
export const status = async (
  ...
  return !request.session.user
    ...
    : response
        .status(200)
        .json({ msg: "Authorized", user: request.session.user });
};
...
export const changeCreds = async (
  ...
  try {
    ...
    const isMatch = await bcrypt.compare(prevPassword, userData[0].password);
